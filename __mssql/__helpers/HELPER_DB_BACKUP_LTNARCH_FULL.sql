...
SELECT @backup_nme =
       (
           SELECT 'D:\Sync\__lanoitan_docs\@db_bak\LTNARCH.BAK'  --'Z:\__ltn_bak\LTNARCH.BAK'
       );

BACKUP DATABASE LTNARCH TO DISK = @backup_nme WITH NOFORMAT, INIT, SKIP, NOREWIND, NOUNLOAD, COMPRESSION, STATS = 25;
...
select [LineNo] as nbr, line 
  from  ltn.Dbo.uft_ReadFileAsTable('D:\Sync\__lanoitan_docs\@db_bak','LTNARCH_sqlbak.log') 
 where 1 = 1
...
+ '----------------------------' + CHAR(13)+CHAR(10)
+ 'Database LTNARCH : backed up on ' + upper(cast(getdate() as varchar(23))) + CHAR(13)+CHAR(10)
+ CHAR(13)+CHAR(10)
...
exec sp_WriteStringToFile @trgt_txt , 'D:\Sync\__lanoitan_docs\@db_bak\','LTNARCH_sqlbak.log'
GO
