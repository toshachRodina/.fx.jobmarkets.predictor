/*
=======================================================================================================================================================================================================
SUBJECT		 : jobscrabe web data from python/beautifulsoup
OBJECT TYPE	 : table
OBJECT NAME     : ltn.dbo.f_dtl_job_scrape_d
...
DROP TABLE ltn.dbo.f_dtl_job_scrape_d;

CREATE TABLE LTN.DBO.F_DTL_JOB_SCRAPE_D
...
, DATA_SRCE   varchar(10) NOT NULL
);

CREATE NONCLUSTERED INDEX UX_F_DTL_JOB_SCRAPE_D ON LTN.DBO.F_DTL_JOB_SCRAPE_D ( MSMT_DTE_ID, CNTRY_CDE, SITE_CDE );

ALTER TABLE ltn.dbo.f_dtl_job_scrape_d REBUILD
...
IF OBJECT_ID('create_f_dtl_job_scrape_d') IS NULL -- Check if SP Exists
    EXEC ('CREATE PROCEDURE create_f_dtl_job_scrape_d AS SET NOCOUNT ON;'); -- Create dummy/empty SP
GO

ALTER PROCEDURE create_f_dtl_job_scrape_d -- Alter the SP Always
(
      @p_env      nvarchar(10)
...
*/
         ...
         SET @l_tbl_nme = 'f_dtl_job_scrape_d';
...
*/
		  ...
		  CREATE TABLE #TMP_DTL_JOB_SCRAPE_D
		  ( MSMT_DTE_ID int NOT NULL
		  , STARTED_AT  datetime NOT NULL
		  , FINISHED_AT datetime NOT NULL
		  , DATA_TYPE   varchar(30) NOT NULL
		  ...
		  INSERT INTO #TMP_DTL_JOB_SCRAPE_D ( MSMT_DTE_ID
									 , STARTED_AT
									 , FINISHED_AT
									 , DATA_TYPE
									 , CNTRY_CDE
...
*/
		  ...
		  INSERT INTO LTN.DBO.F_DTL_JOB_SCRAPE_D ( MSMT_DTE_ID
										 , STARTED_AT
										 , FINISHED_AT
										 , DATA_TYPE
										 , CNTRY_CDE
		  ...
		  SELECT TMP.MSMT_DTE_ID
			  ...
			  , TMP.FACET_CNT
			  , TMP.DATA_SRCE
		    FROM #TMP_DTL_JOB_SCRAPE_D AS TMP
			    LEFT OUTER JOIN LTN.DBO.F_DTL_JOB_SCRAPE_D AS DTL
			    ON 1 = 1
