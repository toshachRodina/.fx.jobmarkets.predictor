/*
=======================================================================================================================================================================================================
SUBJECT		 : anz job ads data from anz website 
OBJECT TYPE	 : table
OBJECT NAME     : ltn.dbo.f_dtl_job_au_anz_scrape_m
...
REMARKS		 : 1) this differs from f_dtl_job_au_anz_scrape_actual_m in the following ways
                     - this takes the pdf data as is and stores the history for each PDF with ADJUSTMENTS
				 - As each period should be a replica of the PDF data, that visual can be replicated if required with this table
				 - f_dtl_job_au_anz_scrape_actual_m will store the most recent data from month to month WITHOUT ADJUSTMENTS
			   2) 
			   3)
INDEXES
...
DROP TABLE ltn.dbo.f_dtl_job_au_anz_scrape_m;

CREATE TABLE LTN.DBO.F_DTL_JOB_AU_ANZ_SCRAPE_M
...
CREATE NONCLUSTERED INDEX UX_F_DTL_JOB_AU_ANZ_SCRAPE_M ON LTN.DBO.F_DTL_JOB_AU_ANZ_SCRAPE_M ( MSMT_DTE_ID );

ALTER TABLE ltn.dbo.f_dtl_job_au_anz_scrape_m REBUILD
...
IF OBJECT_ID('create_f_dtl_job_au_anz_scrape_m') IS NULL -- Check if SP Exists
    EXEC ('CREATE PROCEDURE create_f_dtl_job_au_anz_scrape_m AS SET NOCOUNT ON;'); -- Create dummy/empty SP
GO

ALTER PROCEDURE create_f_dtl_job_au_anz_scrape_m -- Alter the SP Always
(
      @p_env      nvarchar(10)
...
*/
         ...
         SET @l_tbl_nme = 'f_dtl_job_au_anz_scrape_m';
	    SET @l_stg_tbl_nme = 'stg_dtl_job_scrape_d';
...
*/
    ...
    CREATE TABLE #TMP_DTL_JOB_AU_ANZ_SCRAPE_M
    ( MSMT_DTE_ID                       int NOT NULL
    , DATA_TYPE                         varchar(30) NOT NULL
    , CNTRY_CDE                         varchar(2) NOT NULL
    , SITE_CDE                          varchar(30) NOT NULL
    ...
    INSERT INTO #TMP_DTL_JOB_AU_ANZ_SCRAPE_M ( MSMT_DTE_ID
									, DATA_TYPE
									, CNTRY_CDE
									, SITE_CDE
									, RPT_DTE_ID
...
*/
    INSERT INTO LTN.DBO.F_DTL_JOB_AU_ANZ_SCRAPE_M ( MSMT_DTE_ID
										, DATA_TYPE
										, CNTRY_CDE
										, SITE_CDE
										, RPT_DTE_ID
    ...
    SELECT TMP.MSMT_DTE_ID
	    ...
	    , TMP.STARTED_AT
	    , TMP.FINISHED_AT
	 FROM #TMP_DTL_JOB_AU_ANZ_SCRAPE_M AS TMP
		 LEFT OUTER JOIN LTN.DBO.F_DTL_JOB_AU_ANZ_SCRAPE_M AS DTL
		 ON 1 = 1
