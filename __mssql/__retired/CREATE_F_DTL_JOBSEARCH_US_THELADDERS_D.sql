...
OBJECT TYPE	 : table
OBJECT NAME     : ltn.dbo.f_dtl_jobsearch_us_theladders_d
CREATED BY	 : Harold Delaney
CREATED ON	 : 20161201
SOURCE		 : jora [ https://www.theladders.com/ ]
...
TABLE CREATION
-----------------------------
DROP TABLE ltn.dbo.stg_jobsearch_us_theladders_raw;

CREATE TABLE ltn.dbo.stg_jobsearch_us_theladders_raw
             (
...
DROP TABLE ltn.dbo.f_dtl_jobsearch_us_theladders_d;

CREATE TABLE ltn.dbo.f_dtl_jobsearch_us_theladders_d
             (
...
CREATE UNIQUE INDEX ux_f_dtl_jobsearch_us_theladders_d ON ltn.dbo.f_dtl_jobsearch_us_theladders_d(msmt_dte_id, job_category);

ALTER TABLE ltn.dbo.f_dtl_jobsearch_us_theladders_d REBUILD
...
IF OBJECT_ID('create_f_dtl_jobsearch_us_theladders_d') IS NULL -- Check if SP Exists
    EXEC ('CREATE PROCEDURE create_f_dtl_jobsearch_us_theladders_d AS SET NOCOUNT ON;'); -- Create dummy/empty SP
GO

ALTER PROCEDURE create_f_dtl_jobsearch_us_theladders_d -- Alter the SP Always
(
      @p_env      nvarchar(10)
...
-- TABLES ----------------------------------------------------------------
	    ...
	    SET @l_tbl_nme = 'f_dtl_jobsearch_us_theladders_d';
         ...
         SET @l_msmt_dte_id =
             (
                 SELECT MAX(msmt_dte_id)
                        FROM
                             ltn.dbo.stg_jobsearch_us_theladders_raw
             );
...
*/
		  ...
		  SET @l_job_site_nme = 'THELADDERS';

		  INSERT INTO LTN.DBO.F_DTL_JOBSEARCH_US_THELADDERS_D ( MSMT_DTE_ID
												    , JOB_SITE_NME
												    , JOB_CATEGORY
												    , JOB_COUNT
												    )
		  SELECT STG.MSMT_DTE_ID
		    ...
		    FROM LTN.DBO.STG_JOBSEARCH_US_THELADDERS_RAW AS STG
		   WHERE 1 = 1
