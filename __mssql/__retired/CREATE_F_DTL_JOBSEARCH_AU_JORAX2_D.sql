/*
=======================================================================================================================================================================================================
SUBJECT		 : joraX2 - job search site summary job listing data by work category
OBJECT TYPE	 : table
OBJECT NAME     : ltn.dbo.f_dtl_jobsearch_au_jorax2_d
...
TABLE CREATION
-----------------------------
DROP TABLE ltn.dbo.stg_jobsearch_au_jorax2_raw;

CREATE TABLE ltn.dbo.stg_jobsearch_au_jorax2_raw
             (
...
DROP TABLE ltn.dbo.f_dtl_jobsearch_au_jorax2_d;

CREATE TABLE ltn.dbo.f_dtl_jobsearch_au_jorax2_d
             (
...
CREATE UNIQUE INDEX ux_f_dtl_jobsearch_au_jorax2_d ON ltn.dbo.f_dtl_jobsearch_au_jorax2_d(msmt_dte_id, job_category);

ALTER TABLE ltn.dbo.f_dtl_jobsearch_au_jorax2_d REBUILD
...
IF OBJECT_ID('create_f_dtl_jobsearch_au_jorax2_d') IS NULL -- Check if SP Exists
    EXEC ('CREATE PROCEDURE create_f_dtl_jobsearch_au_jorax2_d AS SET NOCOUNT ON;'); -- Create dummy/empty SP
GO

ALTER PROCEDURE create_f_dtl_jobsearch_au_jorax2_d -- Alter the SP Always
(
      @p_env      nvarchar(10)
...
-- TABLES ----------------------------------------------------------------
	    ...
	    SET @l_tbl_nme = 'f_dtl_jobsearch_au_jorax2_d';
         ...
         SET @l_msmt_dte_id =
             (
                 SELECT MAX(msmt_dte_id)
                        FROM
                             ltn.dbo.stg_jobsearch_au_jorax2_raw
             );
...
*/
		  ...
		  SET @text_to_remove_job_category_1 =
			 (
				SELECT rir.val_1
					  ...
					  WHERE
						   1 = 1
						   AND rir.itm_type_cde = 'JORAX2'
						   AND rir.itm_rltshp_type_cde = 'AU_JORAX2'
		  ...
		  SET @l_job_site_nme = 'JORAX2';
		  
		  INSERT INTO ltn.dbo.f_dtl_jobsearch_au_jorax2_d
			    (msmt_dte_id
			   , job_site_nme
			   , job_category
			   , job_count
		  ...
		  SELECT stg.msmt_dte_id
			  ...
			  , LTRIM(RTRIM(REPLACE(REPLACE(UPPER(stg.job_category), @text_to_remove_job_category_1, ''), @text_to_remove_job_category_2, ''))) AS job_category
			  , AVG(CAST([dbo].[fn_getnumeric]( stg.job_count) AS int)) AS job_count
			    FROM
				    ltn.dbo.stg_jobsearch_au_jorax2_raw AS stg
			    WHERE 1 = 1
			    GROUP BY stg.msmt_dte_id
					 , LTRIM(RTRIM(REPLACE(REPLACE(UPPER(stg.job_category), @text_to_remove_job_category_1, ''), @text_to_remove_job_category_2, '')));
