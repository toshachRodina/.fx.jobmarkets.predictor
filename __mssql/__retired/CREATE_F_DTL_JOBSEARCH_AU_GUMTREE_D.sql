...
OBJECT TYPE	 : table
OBJECT NAME     : ltn.dbo.f_dtl_jobsearch_au_gumtree_d
...
DROP TABLE ltn.dbo.stg_jobsearch_au_gumtree_raw;

CREATE TABLE ltn.dbo.stg_jobsearch_au_gumtree_raw
             (
             msmt_dte_id nvarchar ( 8 ) not null
		   ,job_category varchar(max) NOT NULL
             );

DROP TABLE ltn.dbo.f_dtl_jobsearch_au_gumtree_d;

CREATE TABLE ltn.dbo.f_dtl_jobsearch_au_gumtree_d
             (
...
CREATE UNIQUE INDEX ux_f_dtl_jobsearch_au_gumtree_d ON ltn.dbo.f_dtl_jobsearch_au_gumtree_d(msmt_dte_id, job_category);

ALTER TABLE ltn.dbo.f_dtl_jobsearch_au_gumtree_d REBUILD
...
IF OBJECT_ID('create_f_dtl_jobsearch_au_gumtree_d') IS NULL -- Check if SP Exists
    EXEC ('CREATE PROCEDURE create_f_dtl_jobsearch_au_gumtree_d AS SET NOCOUNT ON;'); -- Create dummy/empty SP
GO

ALTER PROCEDURE create_f_dtl_jobsearch_au_gumtree_d -- Alter the SP Always
(
      @p_env      nvarchar(10)
...
=====================================================================================================================================================================================================*/
         ...
         SET @l_tbl_nme = 'f_dtl_jobsearch_au_gumtree_d';
         ...
         SET @l_msmt_dte_id =
             (
                 SELECT MAX(msmt_dte_id)
                        FROM
                             ltn.dbo.stg_jobsearch_au_gumtree_raw
             );
...
=================================================================================================*/
             ...
             CREATE TABLE #tmp_dtl_jobsearch_au_gumtree_s1
                          (
                          msmt_dte_id  int NOT NULL
                        , job_category varchar(255) NOT NULL
                          );
             ...
             WITH row_dat(msmt_dte_id
                  ...
                  SELECT stg.msmt_dte_id
                       ...
                       , STUFF(UPPER(REPLACE(LTRIM(RTRIM(REPLACE(stg.job_category, 'Most visited job categories', ''))), CHAR(13)+' ', '~')), 1, CHARINDEX('~', UPPER(REPLACE(LTRIM(RTRIM(REPLACE(stg.job_category, 'Most visited job categories', ''))), CHAR(13)+' ', '~'))+'~'), '') AS job_category
                         FROM
                              ltn.dbo.stg_jobsearch_au_gumtree_raw AS stg
                  UNION ALL
                  ...
                  INSERT INTO #tmp_dtl_jobsearch_au_gumtree_s1
                         (msmt_dte_id
                        , job_category
                         )
                  SELECT dat.msmt_dte_id
...
=================================================================================================*/
		   ...
		   SET @l_job_site_nme = 'GUMTREE';

             INSERT INTO ltn.dbo.f_dtl_jobsearch_au_gumtree_d
                    (msmt_dte_id
			    , job_site_nme
                   , job_category
                   , job_count
             ...
             SELECT s1.msmt_dte_id
		        , @l_job_site_nme as job_site_nme
                  ...
                  , [dbo].[fn_getnumeric]( s1.job_category ) AS job_count
                    FROM
                         #tmp_dtl_jobsearch_au_gumtree_s1 AS s1
                    WHERE 1 = 1;
