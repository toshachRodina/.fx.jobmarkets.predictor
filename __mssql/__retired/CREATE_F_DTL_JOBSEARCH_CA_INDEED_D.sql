...
OBJECT TYPE	 : table
OBJECT NAME     : ltn.dbo.f_dtl_jobsearch_ca_indeed_d
CREATED BY	 : Harold Delaney
CREATED ON	 : 20161205
SOURCE		 : jora [ http://ca.indeed.com/ ]
...
DROP TABLE ltn.dbo.stg_jobsearch_ca_indeed_raw;

CREATE TABLE ltn.dbo.stg_jobsearch_ca_indeed_raw
             (
...
DROP TABLE ltn.dbo.f_dtl_jobsearch_ca_indeed_d;

CREATE TABLE ltn.dbo.f_dtl_jobsearch_ca_indeed_d
             (
...
CREATE UNIQUE INDEX ux_f_dtl_jobsearch_ca_indeed_d ON ltn.dbo.f_dtl_jobsearch_ca_indeed_d(msmt_dte_id, job_category);

ALTER TABLE ltn.dbo.f_dtl_jobsearch_ca_indeed_d REBUILD
...
IF OBJECT_ID('create_f_dtl_jobsearch_ca_indeed_d') IS NULL -- Check if SP Exists
    EXEC ('CREATE PROCEDURE create_f_dtl_jobsearch_ca_indeed_d AS SET NOCOUNT ON;'); -- Create dummy/empty SP
GO

ALTER PROCEDURE create_f_dtl_jobsearch_ca_indeed_d -- Alter the SP Always
(
      @p_env      nvarchar(10)
...
-- TABLES ----------------------------------------------------------------
	    ...
	    SET @l_tbl_nme = 'f_dtl_jobsearch_ca_indeed_d';
         ...
         SET @l_msmt_dte_id =
             (
                 SELECT MAX(msmt_dte_id)
                        FROM
                             ltn.dbo.stg_jobsearch_ca_indeed_raw
             );
...
*/
			 ...
			 CREATE TABLE #tmp_dtl_jobsearch_ca_indeed_s1
					    (
					    msmt_dte_id  int NOT NULL
					  , job_site_nme varchar(25) NOT NULL
					  , job_category varchar(255) NOT NULL
				...
				(
				    SELECT rir.val_1
						  ...
						  WHERE 1 = 1
								...
								AND rir.itm_rltshp_type_cde = 'CA_INDEED'
			 ...
			 INSERT INTO #tmp_dtl_jobsearch_ca_indeed_s1
				    (msmt_dte_id
				    , job_site_nme
				    , job_category
				    , job_count
			 ...
			 SELECT stg.msmt_dte_id
				    ...
				    , LTRIM(RTRIM(REPLACE(UPPER(stg.job_category), @text_to_remove_industry_field, ''))) AS job_category
				    , AVG(CAST([dbo].[fn_getnumeric]( stg.job_count ) AS int)) AS job_count
				    FROM
					   ltn.dbo.stg_jobsearch_ca_indeed_raw AS stg
				    WHERE 1 = 1
				    GROUP BY stg.msmt_dte_id
						  , LTRIM(RTRIM(REPLACE(UPPER(stg.job_category), @text_to_remove_industry_field, '')))
				    ORDER BY 3;
/*
STEP 2
---------------------------------------------------------------------------------------------------
...
*/
			 ...
			 INSERT INTO ltn.dbo.f_dtl_jobsearch_ca_indeed_d
				   (msmt_dte_id
				  , job_site_nme
				  , job_category
				  , job_count
			 ...
			 SELECT s1.msmt_dte_id
				 , s1.job_site_nme
				 , s1.job_category
				 , s1.job_count AS job_count
				   FROM
					   #tmp_dtl_jobsearch_ca_indeed_s1 AS s1
				   WHERE 1 = 1;
