...
OBJECT TYPE	 : table
OBJECT NAME     : ltn.dbo.f_dtl_jobsearch_au_hays_d
...
DROP TABLE ltn.dbo.stg_jobsearch_au_hays_raw;

CREATE TABLE ltn.dbo.stg_jobsearch_au_hays_raw
             (
...
DROP TABLE ltn.dbo.f_dtl_jobsearch_au_hays_d;

CREATE TABLE ltn.dbo.f_dtl_jobsearch_au_hays_d
             (
...
CREATE UNIQUE INDEX ux_f_dtl_jobsearch_au_hays_d ON ltn.dbo.f_dtl_jobsearch_au_hays_d(msmt_dte_id, job_category);

ALTER TABLE ltn.dbo.f_dtl_jobsearch_au_hays_d REBUILD
...
IF OBJECT_ID('create_f_dtl_jobsearch_au_hays_d') IS NULL -- Check if SP Exists
    EXEC ('CREATE PROCEDURE create_f_dtl_jobsearch_au_hays_d AS SET NOCOUNT ON;'); -- Create dummy/empty SP
GO

ALTER PROCEDURE create_f_dtl_jobsearch_au_hays_d -- Alter the SP Always
(
      @p_env      nvarchar(10)
...
-- TABLES ----------------------------------------------------------------
	    ...
	    SET @l_tbl_nme = 'f_dtl_jobsearch_au_hays_d';
         ...
         SET @l_msmt_dte_id =
             (
                 SELECT MAX(msmt_dte_id)
                        FROM
                             ltn.dbo.stg_jobsearch_au_hays_raw
             );
...
*/
             ...
             CREATE TABLE #tmp_dtl_jobsearch_au_hays_s1
                          (
                          msmt_dte_id  int NOT NULL
                        , job_site_nme varchar (25) NOT NULL
                        , job_category varchar(255) NOT NULL
                 ...
                 (
                     SELECT rir.val_1
                            ...
                            WHERE 1 = 1
                                  ...
                                  AND rir.itm_rltshp_type_cde = 'AU_HAYS'
	    ...
	    SET @l_job_site_nme = 'HAYS';

insert into #tmp_dtl_jobsearch_au_hays_s1
(
        msmt_dte_id
      , job_site_nme
      , job_category
...
select
      ...
      ,dbo.fn_getnumeric ( replace ( upper ( stg.job_count ) , @text_to_remove_job_count,'' )) as job_count
       from ltn.dbo.stg_jobsearch_au_hays_raw as stg;
...
insert into ltn.dbo.f_dtl_jobsearch_au_hays_d
(
        msmt_dte_id
      , job_site_nme
      , job_category
...
select
      ...
      , s1.job_category
      , s1.job_count
       from #tmp_dtl_jobsearch_au_hays_s1 as s1
       where 1 = 1;
