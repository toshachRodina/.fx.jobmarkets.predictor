...
OBJECT TYPE	 : table
OBJECT NAME     : ltn.dbo.f_dtl_jobsearch_ca_careerbuilder_d
CREATED BY	 : Harold Delaney
CREATED ON	 : 20161205
SOURCE		 : gov [ http://www.careerbuilder.ca/ ]
...
DROP TABLE ltn.dbo.stg_jobsearch_ca_careerbuilder_raw;

CREATE TABLE ltn.dbo.stg_jobsearch_ca_careerbuilder_raw
             (
             msmt_dte_id  nvarchar(8) NOT NULL
           , job_category varchar(max) NOT NULL
             );

DROP TABLE ltn.dbo.f_dtl_jobsearch_ca_careerbuilder_d;

CREATE TABLE ltn.dbo.f_dtl_jobsearch_ca_careerbuilder_d
             (
...
CREATE UNIQUE INDEX ux_f_dtl_jobsearch_ca_careerbuilder_d ON ltn.dbo.f_dtl_jobsearch_ca_careerbuilder_d(msmt_dte_id, job_category);

ALTER TABLE ltn.dbo.f_dtl_jobsearch_ca_careerbuilder_d REBUILD
...
IF OBJECT_ID('create_f_dtl_jobsearch_ca_careerbuilder_d') IS NULL -- Check if SP Exists
    EXEC ('CREATE PROCEDURE create_f_dtl_jobsearch_ca_careerbuilder_d AS SET NOCOUNT ON;'); -- Create dummy/empty SP
GO

ALTER PROCEDURE create_f_dtl_jobsearch_ca_careerbuilder_d -- Alter the SP Always
(
      @p_env      nvarchar(10)
...
=====================================================================================================================================================================================================*/
         ...
         SET @l_tbl_nme = 'f_dtl_jobsearch_ca_careerbuilder_d';
         ...
         SET @l_msmt_dte_id =
             (
                 SELECT MAX(msmt_dte_id)
                        FROM
                             ltn.dbo.stg_jobsearch_ca_careerbuilder_raw
             );
...
=================================================================================================*/
             ...
             CREATE TABLE #tmp_dtl_jobsearch_ca_careerbuilder_s1
                          (
                          msmt_dte_id  int NOT NULL
                        , job_category varchar(255) NOT NULL
                          );
             ...
             WITH row_dat(msmt_dte_id
                 ...
                 FROM
                              ltn.dbo.stg_jobsearch_ca_careerbuilder_raw AS stg
                  UNION ALL
                  ...
                  INSERT INTO #tmp_dtl_jobsearch_ca_careerbuilder_s1
                         (msmt_dte_id
                        , job_category
                         )
                  SELECT dat.msmt_dte_id
...
=================================================================================================*/
			 ...
			 INSERT INTO ltn.dbo.f_dtl_jobsearch_ca_careerbuilder_d
				   (msmt_dte_id
				  , job_site_nme
				  , job_category
				  , job_count
			 ...
			 SELECT s1.msmt_dte_id
				 ...
				 , [dbo].[fn_getnumeric]( s1.job_category ) AS job_count
				   FROM
					   #tmp_dtl_jobsearch_ca_careerbuilder_s1 AS s1
				   WHERE 1 = 1
					    AND LTRIM(RTRIM(s1.job_category)) NOT IN (
														'SHOW MORE'
													   , 'SHOW LESS'
														)
				   ORDER BY 3;
