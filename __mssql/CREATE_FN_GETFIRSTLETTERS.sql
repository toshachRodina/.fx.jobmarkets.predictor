...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[fn_getfirstletters]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [dbo].[fn_getfirstletters]
GO

CREATE FUNCTION dbo.[fn_getfirstletters]
                (
     ...
     BEGIN
         ...
         WITH cte
              ...
              SELECT @firstletter = STUFF(
                     (
                         SELECT ''+LEFT([column], @number_of_letters)
                                FROM
                                     cte
                                FOR xml PATH('')
                     ), 1, 0, '');
