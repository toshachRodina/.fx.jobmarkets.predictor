...
def main():
    init()
    # ONLY RUN ON THE 10th DAY OF EACH MONTH
    if int(g['DAY_NBR']) == 10:
        get_vars()
        email_status('START')
        script_run()
...
def email_status(step):
    if step == 'START':
        # SUBJECT & RECIPIENTS
        mymail = pyMail('CCY PAIRS HISTORY' + ' - STARTED @ ' + time.strftime("%Y-%m-%d %H:%M:%S"), **g)
...
#         htmlRes = htmlRes + '</table>'
        # SUBJECT & RECIPIENTS
        mymail = pyMail('CCY PAIRS HISTORY' + ' - ENDED @ ' + time.strftime("%Y-%m-%d %H:%M:%S"), **g)
        # START HTML BODY (GREETING / OPENING LINE OF EMAIL)
        # FURTHER DETAILS ADDED TO BODY (SEPERATED BY A PARAGRAPH SO LINE FEEDS NOT REQUIRED)
        # ADD LINE OF TEXT
        mymail.htmladd('Download has completed for : ' + 'CCY PAIRS HISTORY')
