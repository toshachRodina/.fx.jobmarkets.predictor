...
def scrape():
    ...
    for div in soup('div',class_="sbus-home-cont"):
        for tr in div.find_all('tr'):
            
            if '>INDUSTRY<' in str(tr).upper():
                facet_type = 'INDUSTRY'
            elif '>MAJOR CITY<' in str(tr).upper():
                facet_type = 'MAJOR CITY'
            else:
                facet_type = 'NOT IDENTIFIED'
              
            for a in tr.find_all('a'):
                time.sleep(rndm_sleep) # insert a pause to assist in reflecting human usage of webpage
                url_sub = str(a.get('href'))
                # =============================================================================
                # PASS URL TO RETURN HTML FROM SITE PAGE
                # CAPTURE ANY ERRORS EXPERIENCED AND PASS TO LOCAL DB
                # =============================================================================
                ...
                soup = BeautifulSoup(passedHTML, "html.parser")
                #print(soup)                
                facet_desc = a.text.upper().replace(r"'",'')
                #print(facet_desc)
                
                for span in soup.find_all('span',class_="count"):
                    nbr = span.text.upper().replace(',','')
