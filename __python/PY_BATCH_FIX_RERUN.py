...
# LIBRARY AND DATABASE FILE PATH
utilPath = os.path.join(path, '../__python/__utils/')
utilPath = os.path.abspath(os.path.realpath(utilPath))  
dbPath = os.path.join(path, '../__python/@data/')
...
def script_run():
    ...
    try:
        for pkg in log_vars:
            ...
            subprocess.call([pyPath, str(tempDict['PKG_PATH'] + r"\\" + pkg + '.py')])
    except:
...
def email_status(step):
    if step == 'START':
        # SUBJECT & RECIPIENTS
        mymail = pyMail(g['PKG_NME_PRNT'] + ' : AU - STARTED @ ' + time.strftime("%Y-%m-%d %H:%M:%S"), **g)
        ...
        # =============================================================================   
        dbmgr = pyDB(g['DB'])
        q = r"""SELECT msmt_dte_id, cntry_cde, count( * ) AS row_cnt, sum(facet_cnt) as job_count FROM {0} WHERE cntry_cde = 'AU' GROUP BY msmt_dte_id, cntry_cde ORDER BY msmt_dte_id DESC LIMIT 5""".format(
            'PY_JOBS_DATA'
            ) 
        rslt = dbmgr.query(q)
        # =============================================================================
        ...
        # SUBJECT & RECIPIENTS
        mymail = pyMail(g['PKG_NME_PRNT'] + ' : AU - ENDED @ ' + time.strftime("%Y-%m-%d %H:%M:%S"), **g)
        # START HTML BODY (GREETING / OPENING LINE OF EMAIL)
