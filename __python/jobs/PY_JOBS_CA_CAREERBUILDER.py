...
# OBJECT NAME    : py_jobs_ca_careerbuilder
# CREATED BY     : Harold Delaney
# CREATED ON     : 20170212
# SOURCE         : gumtree [ http://www.careerbuilder.ca/ ]
...
def scrape():
    ...
    for div in soup.find_all('div', class_='facet'):
        for a in div.find_all('a'):
            link_txt = a.text.upper()
            link_all = str(a)
            link_all = re.search(r"\'Facet[^s]([A-z]*)?'", link_all).group()
            facet_type = link_all.upper().replace('FACET','').replace(r"'",'')
            
            if facet_type == 'EMPLOYMENTTYPE':
                facet_type = 'JOB TYPE'
            ...
            facet_desc = link_txt.encode("utf-8","ignore").decode('ascii', 'ignore')
            facet_desc = facet_desc.replace(r"'",'')
            
        for span in div.find_all('span'):
            span_txt = span.text
            facet_count = span_txt.replace(r'(','').replace(r')','')
        # =============================================================================
