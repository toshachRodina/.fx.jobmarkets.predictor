...
def scrape():
    ...
    facet_count = nbr[0]
    facet_count = int(float(facet_count.replace(',','')))
    # =============================================================================
    ...
    for ul in soup.find_all('ul', class_='facet'):
        for li in ul.find_all('li'):
            ...
            if facet_desc:  # checks if there is a result on the search for the "a" anchor (removes the title of the sections by default - returned above) 
                ...
                # SITE STOPPED INCLUDING VALUES IN THE COUNT SECTION
                # TEST IF THERE - IF NOT, LINK TO RELEVANT PAGE AND RETURN COUNT                
                try:
                    ...
                    soup = BeautifulSoup(passedHTML, "html.parser")
                    #print(str(soup.encode("utf-8")))
                    for div in soup.find_all('div', class_='column page-entries-info'):
                        facet_count = re.search(' OF <SPAN>(.*?)</SPAN>', str(soup.encode("utf-8")).upper()).group(1)
