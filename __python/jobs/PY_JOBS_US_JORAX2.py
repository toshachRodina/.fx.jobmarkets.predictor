...
# OBJECT NAME    : py_jobs_us_joraX2
# CREATED BY     : Harold Delaney
# CREATED ON     : 20170220
# SOURCE         : jora [ https://us.jora.com ]
...
# FREQUENCY      : DAILY
#               
# REMARKS        : 1) jora has a top industries list which is covered in similar script
#                  2) jorax2 is every industry classification and returns significantly more job counts
#                  3)
# =======================================================================================================================================================================================================
# =============================================================================
        ...
        for a in div.find_all('a'):
            ...
            for h1 in soup.find_all('h1'):
                for a in h1.find_all('a'):
                    ...
                    txt = a.text.upper()
                    facet_desc = txt.replace(r"'",'').replace('JOBS','').strip()
                    # =============================================================================
                    # PASS URL TO RETURN HTML FROM SITE PAGE
                    # CAPTURE ANY ERRORS EXPERIENCED AND PASS TO LOCAL DB
                    # =============================================================================
                    url = g['URL'] + r'/' + dest_url2
                    passedHTML = pyHTMLPass.htmlPass(url,**g)
                    soup = BeautifulSoup(passedHTML, "html.parser")
                    
                    nbr = re.search('</SPAN> OF <SPAN>(.*?)</SPAN>', str(soup).encode("utf-8","ignore").decode('ascii', 'ignore').upper()).group(1)
                    nbr = str(nbr).replace(',','')
                    facet_count = int(nbr)
                    # =============================================================================
