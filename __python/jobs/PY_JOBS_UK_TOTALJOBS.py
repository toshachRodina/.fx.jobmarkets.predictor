...
# OBJECT NAME    : py_jobs_uk_totaljobs
# CREATED BY     : Harold Delaney
# CREATED ON     : 20170216
# SOURCE         : totaljobs [ https://www.totaljobs.com/ ]
...
def scrape():
    ...
    for section in soup.find_all('section', id='tab-list'):
        for div in section.find_all('div'):            
            for span in div.find_all('span', class_='title'):
                sp = span.text.upper()
                sp = sp.replace('JOBS BY','')
                facet_type = sp.strip()            
            for li in div.find_all('li'):                
                try:
                    li_txt = re.search('\">(.*?)</a>', str(li)).group(1)
                    facet_desc = li_txt.replace(',','').upper()
                
                    li_nbr = re.search('<span>(.*?)</span>', str(li)).group(1)
                    facet_count = int(li_nbr.replace('(','').replace(')','').replace(',',''))
                except:
                    None               
                
                if facet_type == 'POPULAR SEARCHES' or facet_type == 'COMPANY' or 'SEE MORE' in facet_desc or 'VIEW ALL' in facet_desc:
                    None
                else:
                    # =============================================================================
