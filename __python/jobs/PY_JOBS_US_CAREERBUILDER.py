...
# OBJECT NAME    : py_jobs_us_careerbuilder
# CREATED BY     : Harold Delaney
# CREATED ON     : 20170213
# SOURCE         : careerbuilder [ http://www.careerbuilder.ca/ ]
...
# REMARKS        : 1) NO API
#                  2) ERROR:  
#                     ((class 'urllib.error.URLError'), URLError(SSLError(1, '[SSL: CERTIFICATE_VERIFY_FAILED] certificate verify failed (_ssl.c:749)'),), (traceback object at 0x032E5A80))
#                     RESOLUTION:
...
def scrape():
    ...
    for select in soup.find_all('select', id='cat1'):
        for option in select.find_all('option'):            
            ...
            nbr_chk = re.findall('\([0-9]*\)', str(option)) # FIND ANY WITHIN BRACKETS            
            if nbr_chk:
                nbr = nbr_chk[0]
                nbr = str(nbr)
                facet_count = int(nbr.replace('(','').replace(')',''))
                
                txt = re.findall(r'">(.+?)</OPTION>', str(option).upper()) # find any within stated regex
                txt = txt[0]
                txt = str(txt)
                txt = re.sub('\d', '', txt)
