        ...
        for i in range(10):
            ...
            while True:
                try:
                    ...
                    #print(soup)            
                    #facet_count = re.search('PAGE 1 OF(.*?)</DIV>', str(soup).encode("utf-8","ignore").decode('ascii', 'ignore').upper()).group(1)  
                    facet_count = re.search(r'PAGE 1 OF(.*?)JOBS</DIV>', str(soup).encode("utf-8","ignore").decode('ascii', 'ignore').upper()).group(1)
                    facet_count = int(facet_count.replace(',','').replace('JOBS','').strip())
                except :
                    e = sys.exc_info()
