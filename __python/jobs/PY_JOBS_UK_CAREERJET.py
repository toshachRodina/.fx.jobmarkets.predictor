...
# OBJECT NAME    : py_jobs_uk_careerjet
# CREATED BY     : Harold Delaney
# CREATED ON     : 20170213
# SOURCE         : careerjet [ http://www.careerjet.co.uk/ ]
...
def scrape():
    ...
    for links in soup.find_all('a'):        
        ...
        if link_nbr_:
            ...
            nbr_chk = 0        
        if 'jobs-in-' in link_txt:
            facet_type = 'REGION'
        else:
            facet_type = 'INDUSTRY'        
        # FINAL ASSIGNMENTS
        #dest_url = url + link_txt
        facet_desc = links.string.upper()
        facet_desc = re.sub(r"[!@#$']", '', str(facet_desc)) # removes special characters from string
        
        if nbr_chk < 100 and 'jobs-in-' in link_txt:
            facet_desc = facet_desc + ' (REGION TOTAL)'
        else:
            facet_desc = facet_desc
          
        if 'jobs-' in link_txt and nbr_chk <= int(g['REGION_CHK_ID']): # if href matches what is considered relevant, do the following
            ...
            soup = str(soup)             
            try:
                nbr = re.search(r'1 to(.*?)jobs', soup).group(1)
